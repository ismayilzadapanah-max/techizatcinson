...
export default function AdminUsersPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: suppliers }, { data: restaurants }] = await Promise.all([
      supabase.from("supplier_profiles").select("id, company_name, email, phone, city, approval_status, created_at").order("created_at", { ascending: false }),
      supabase.from("restaurant_profiles").select("id, restaurant_name, email, phone, city, approval_status, created_at").order("created_at", { ascending: false }),
    ]);
    ...
    let combined: UserRow[] = [
      ...((suppliers || []) as { id: string; company_name: string; email: string; phone: string; city: string; approval_status: string; created_at: string }[]).map(s => ({
        id: s.id, name: s.company_name, role: "supplier" as const, email: s.email, phone: s.phone, city: s.city, approval_status: s.approval_status, created_at: s.created_at,
      })),
      ...((restaurants || []) as { id: string; restaurant_name: string; email: string; phone: string; city: string; approval_status: string; created_at: string }[]).map(r => ({
        id: r.id, name: r.restaurant_name, role: "restaurant" as const, email: r.email, phone: r.phone, city: r.city, approval_status: r.approval_status, created_at: r.created_at,
      })),
    ].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

    if (search) combined = combined.filter(u => u.name.toLowerCase().includes(search.toLowerCase()) || u.email.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#5D608B]">{loading ? "Yüklənir..." : `${users.length} istifadəçi`}</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-[#E9E8EE] flex flex-wrap gap-3">
        <div className="relative">
          ...
          <input type="text" placeholder="Ad və ya e-mail axtar..." value={search} onChange={e => setSearch(e.target.value)}
            className="pl-9 pr-4 py-2 bg-[#F3F2F7] border border-[#E9E8EE] rounded-lg text-sm outline-none w-56" />
        </div>
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="bg-[#F3F2F7] border border-[#E9E8EE] rounded-lg px-4 py-2 text-sm outline-none">
          <option value="">Bütün rollar</option>
          <option value="supplier">Təchizatçı</option>
          <option value="restaurant">Restoran</option>
        </select>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-[#F3F2F7] border border-[#E9E8EE] rounded-lg px-4 py-2 text-sm outline-none">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-[#E9E8EE] overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center">
            <span className="material-symbols-outlined text-5xl text-[#E9E8EE] block mb-3">person_search</span>
                ...
                {users.map(u => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-[#F3F2F7] transition-colors">
                      <td className="px-4 py-3 text-sm font-semibold text-[#141647] max-w-[140px] truncate">{u.name}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${u.role === "supplier" ? "bg-blue-100 text-blue-700" : "bg-emerald-100 text-emerald-700"}`}>
                          {u.role === "supplier" ? "Təchizatçı" : "Restoran"}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-[#5D608B] max-w-[160px] truncate">{u.email || "—"}</td>
                      <td className="px-4 py-3 text-sm text-[#5D608B] whitespace-nowrap">{u.phone || "—"}</td>
                      <td className="px-4 py-3 text-sm text-[#5D608B] whitespace-nowrap">{u.city || "—"}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${st.color}`}>{st.label}</span>
                      </td>
                      <td className="px-4 py-3 text-xs text-[#9DB1CA] whitespace-nowrap">{new Date(u.created_at).toLocaleDateString("az-AZ")}</td>
                      <td className="px-4 py-3">
                        <div className="flex gap-1">
                          {u.approval_status !== "approved" && (
                            <button onClick={() => updateStatus(u, "approved")} disabled={busy} title="Təsdiqlə"
                          ...
                          {u.approval_status !== "rejected" && (
                            <button onClick={() => updateStatus(u, "rejected")} disabled={busy} title="Rədd et"
                          ...
                          {u.approval_status !== "disabled" ? (
                            <button onClick={() => updateStatus(u, "disabled")} disabled={busy} title="Deaktiv et"
                              className="p-1.5 bg-gray-50 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-40">
                              <span className="material-symbols-outlined text-[16px]">block</span>
                            </button>
                          ) : (
                            <button onClick={() => updateStatus(u, "pending")} disabled={busy} title="Aktiv et"
