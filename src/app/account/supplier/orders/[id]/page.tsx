...
import { UpdateOrderStatusButton } from "@/components/orders/UpdateOrderStatusButton";
...
export default function SupplierOrderDetailPage() {
  ...
  function handleStatusUpdated(newStatus: OrderStatus) {
    setOrder((prev) => prev ? { ...prev, status: newStatus } : prev);
    load(); // Refresh to get new history entry
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-[#5D608B] mt-0.5">
            Restoran: <strong>{order.restaurantName || "—"}</strong>
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-[#E9E8EE] p-5 space-y-4">
            <h3 className="font-semibold text-[#141647] flex items-center gap-2">
              <span className="material-symbols-outlined text-[18px] text-[#9DB1CA]">edit_note</span>
              Status dəyişdir
            </h3>
            {user ? (
              ...
              <p className="text-sm text-[#5D608B]">Status dəyişikliyi üçün daxil olun.</p>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border border-[#E9E8EE] p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              {order.note && (
                <div className="col-span-2">
                  <p className="text-xs text-[#9DB1CA] mb-0.5">Müştəri qeydi</p>
                  <p className="text-[#5D608B] italic bg-[#F4F5F9] px-3 py-2 rounded-lg">"{order.note}"</p>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Review received */}
          {(order.qualityRating || order.satisfactionScore) && (
            <div className="bg-white rounded-xl border border-[#E9E8EE] p-5 space-y-3">
              ...
              {order.qualityRating && (
                <div className="flex items-center gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <span
                      key={i}
                      className={`material-symbols-outlined text-[22px] ${i < (order.qualityRating ?? 0) ? "text-amber-400" : "text-gray-200"}`}
                    >
                      star
                    </span>
                  ))}
              ...
              {order.satisfactionScore && (
                <p className="text-sm text-[#5D608B]">
                  Məmnuniyyət: <strong className="text-[#141647]">{order.satisfactionScore}/10</strong>
                </p>
              )}
              {order.satisfactionNote && (
                <p className="text-sm text-[#5D608B] italic bg-[#F4F5F9] px-3 py-2 rounded-lg">
                  "{order.satisfactionNote}"
                </p>
              )}
            </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-[#E9E8EE] p-5 space-y-3">
            <h3 className="font-semibold text-[#141647] text-sm">Əlaqəli</h3>
            <Link
              href="/account/supplier/orders"
